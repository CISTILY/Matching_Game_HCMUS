...
void Menu::printAnimation()
{
	...
	while (loop--)
	{
		for (int i = 0; i < n; i += 2)
		{
			Control::setConsoleColor(BRIGHT_WHITE, getRandomInt(0, 15));
...
void Menu::changeOption(bool direction, bool flag) //0: lên, 1: xuống
{
	...
	Control::setConsoleColor(BRIGHT_WHITE, BLACK);
	Control::gotoXY(50 - (int)options[current_option].length() / 2, top + current_option % 4 * 2);
	cout << options[current_option];
	if (flag)
	{
		Control::gotoXY(40, top + current_option % 4 * 2);
		putchar(32);
		Control::gotoXY(60, top + current_option % 4 * 2);
		putchar(32);
	}
